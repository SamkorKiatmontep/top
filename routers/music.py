...
async def add_song(song: MusicCreate):
    async with new_session() as session:
        ...
        return {'id': new_song.id, 'title': new_song.title, 'artist': new_song.artist, 'file_path': new_song.file_path}
...
async def get_one(song_id: int):
    async with new_session() as session:
        ...
        if not song:
            raise HTTPException(404, 'не найден')
        return {'id': song.id, 'title': song.title, 'artist': song.artist, 'file_path': song.file_path}